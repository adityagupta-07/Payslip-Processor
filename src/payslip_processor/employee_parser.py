...
def get_details_per_employee(sheet, block, employee_obj: Employee):
    ...
    for r in range(block[0], block[1] + 1): # r loops from 1 to 20 (exclusive)
        for target_cell in sheet[r]: # target_cell loops through all the cells in row(r), means row(1), row(2), row(3), ...
            ...
            # if target_value in employee_obj.check_attribute(target_value): # Wrong approach to check attribute in obj
            if employee_obj.check_attribute(str(target_value)):
                ...
                if found_ssf_contribution_by_employer is True and "SSF Contribution by Employer" in target_value:
                    employee_obj.set_atr("Lower SSF Contribution by Employer", f"Rs. {next_cell.value:,.2f}")
                    break
                if "SSF Contribution by Employer" in target_value:
                    found_ssf_contribution_by_employer = True
                    employee_obj.set_atr("Upper SSF Contribution by Employer", f"Rs. {next_cell.value:,.2f}")
                    employee_obj.set_atr("Financial_Year_Note", sheet.cell(row=r, column=(target_cell.column+2)).value)
